...
export async function POST(
  ...
  try {
    ...
    if (isTerminalState(runResult.data.state)) {
      return NextResponse.json(
        { error: `Run is already in terminal state: ${runResult.data.state}` },
        { status: 400 }
      )
    }
    ...
    // Perform cancel via state machine
    const cancelResult = stateMachine.performAction(runResult.data, { action: 'cancel' })

    // Update run in database
    const updateResult = await runDB.updateRunState(runId, {
      state: cancelResult.newState,
      previous_state: null, // Cancel is terminal, no previous state needed
    })
